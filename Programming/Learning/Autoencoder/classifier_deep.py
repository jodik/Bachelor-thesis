...
class ClassifierDeepAutoEncoder(AutoencoderDataLoader):
    ...
    def run(self):
        ...
        val_predcs = classifier.predict(self.validation_data if not self.conf_s.USE_TEST_DATA else self.test_data)
        accuracy, confusion_matrix = helper.error_rate(val_predcs, self.validation_labels_o if not self.conf_s.USE_TEST_DATA else self.test_labels_o)
